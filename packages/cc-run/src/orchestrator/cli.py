...
def cmd_run(args: argparse.Namespace) -> int:
    projects = resolve_projects(args)
    if not projects:
        console.print("[yellow]Aucun projet detecte.[/yellow]")
        return 1
...
def cmd_preset(args: argparse.Namespace) -> int:
    ...
    if skipped:
        console.print(
            f"[yellow]Skip (preset {args.preset} indispo) : {', '.join(skipped)}[/yellow]"
        )
    if not jobs:
        console.print("[yellow]Aucun job a lancer.[/yellow]")
        return 1
    results = asyncio.run(run_many(jobs, concurrency=args.concurrency, timeout=args.timeout))
    print_results(results, verbose=args.verbose)
    return 0 if all(r.ok for r in results) else 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--projects", default=None, help="Noms separes par virgule.")
    p.add_argument("--match", default=None, help="Filtre kind : python | node | static")
    ...
    lst = sub.add_parser("list", help="Liste les projets detectes.")
    lst.set_defaults(func=cmd_list)

    run = sub.add_parser("run", help="Exec commande libre sur tous les projets filtres.")
    run.add_argument("command", help="Commande shell.")
    run.set_defaults(func=cmd_run)

    preset = sub.add_parser("preset", help="Exec preset (test/lint/build/status).")
